...
**Script:** [Profiling (script)](../profiling.py) · **Flow:** [diagram](../__flow/profiling.md)
...
### Uses
- [Config (folder)](../___config.md) — `paths.settings_path()` for the
  store's location beside `settings.json`

### Used by
- [Watch Controller](../../app/__about/controller.md) — tick/skin
  instrumentation, the minute flush, quit flush
- [Compositor](../../render/__about/compositor.md) — paint, composite rebuild,
  hit test and hover instrumentation
- [Assets](../../render/__about/assets.md) — subdial recolor, working-set
  warmup
- [Report](../../app/__about/report.md) — `snapshot()`, `reset()`

## Functions

- `timed(name)`: decorator — measures every call of the wrapped
  function under `name` (exceptions still count — the time was spent)
...
- `flush()`: atomic save when dirty (tmp file + `os.replace`)
