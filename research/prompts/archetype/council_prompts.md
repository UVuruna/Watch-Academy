# Council Archetype Prompts — the Six Offices in Session

The PRISM pointer's THIRD wheel (menu: Paint / Light / **Council**),
sealed in [The Cube Canon](../../../CUBE.md) §The Double Trinity: the
UNION — all six offices of the Double Trinity on the six arms at once,
the Court's upward triangle and Genesis's downward triangle interlocked
into the star. Anchor: **Psalm 82:1** — *"God standeth in the
congregation of the mighty; he judgeth among the gods"* — the divine
council in session.

| Arm | Colour | Person | Office | Triangle |
|---|---|---|---|---|
| **12h** | yellow | God | **Judge** | Court (up) |
| **16h** | orange | the Devil | **Destroyer** | Genesis (down) |
| **20h** | red | the Devil | **Prosecutor** | Court (up) |
| **24h** | moon-gray purple (~`#666699`) | God | **Creator** | Genesis (down) |
| **04h** | blue | Jesus | **Advocate** | Court (up) |
| **08h** | green | Jesus | **Preserver** | Genesis (down) |

Three persons, six seats: God holds the vertical (Judge at noon, Creator
at midnight — the G↔Ω diameter of the Banknote, Revelation 1:8), and the
...
register as the Court's lancets ([Trinity Prompts](trinity_prompts.md))
and Genesis's ([Genesis Prompts](genesis_prompts.md)). Each office is a
**TALL pointed-arch lancet** (aspect 1:2) in its arm's colour plus a
**1:1 circle companion** (see
[Badge 1:1 Prompts](../badge/badge_1to1_prompts.md)); the centre is a
round rosette.
...
terminal `_gem`/`_gpt` suffix.

**Drop paths:** `assets/archetypes/council/primary/colored/` and
...
names belong to the articles and the legend.

---

## Rule #19 derivation check (written, root `CLAUDE.md` Rule #19)

The hard question this sheet exists to answer honestly: **six offices
that already have (or are getting) art on two other wheels — is a
Council image a NEW image at all?**

**COMPUTED — never generated:**

- **The interlock itself.** "Both triangles at once" is arm PLACEMENT:
  six files on six arms. No composite "star" image is ever generated.
- **The Diamond/Cube look toggle** (CUBE §Display laws) mounts the SAME
  files into corner-view rhombi — geometry, not a second art set.
- **Hexagram rotation, lit/ghost/centre-burn opacity, metal tints** —
  transform and palette maths, exactly as on every other wheel.
- **The 24-field union table** (12 outer offices, 12 inner processes) is
  Encyclopedia TEXT — twelve more office plates are not art we owe.
- **The Rose of the Twenty-Four gets NO sheet**: three identical octa
  stars at −15°/0°/+15°, 24 rays in 8 palette colours — computed
...
pull opposite ways here and this sheet refuses to resolve it silently:

- The **one-image-one-place law** (owner 2026-07-19, the law that revoked
  the Prism poles' reuse of the Scale glass) says every seat gets its own
  telling; a file serving two wheels is exactly what it forbade.
- **Rule #5 / Rule #19** say the same office by the same person in the
  same register, differing only by which arms its neighbours occupy, is
  not a new irreducible core.

The honest distinction, and the reason a new telling is defensible: on
the Court and Genesis wheels each office stands **alone and acting** — the
Advocate pleading, the Destroyer pulling the keystone. On the Council
every office is **in session**: seated in a ring of six thrones, turned
toward the shared centre, acting *before the others*. That is a different
COMPOSITION of the same person and office — the class of thing this
project does generate. Both routes are written out below; the owner picks
ONE.

- **Route A (written in full, default if the owner says nothing):** six
  NEW "in session" lancets + six circles, 12 files, drop paths as stated.
- **Route B — REUSE, PENDING OWNER:** generate NOTHING but the centre;
  `config.archetypes["prism_council"]` points its six figures at the
  existing files —
  `archetypes/trinity/primary/colored/{One_Judge,Devil_Prosecutor,Jesus_Advocate}.png`
  and `archetypes/genesis/primary/colored/{God_Creator,Jesus_Preserver,Devil_Destroyer}.png`
  (and the matching `circle/colored/` companions). Technically free — the
  code reads a path, and nothing stops two archetype groups naming the
  same one. The Council then differs from Court + Genesis only in showing
  all six at once, which is arguably the entire point of a UNION wheel.

**Route A totals 13 files; Route B totals 1.** Route A's whole section carries a
standalone skip marker, so PromptPainter loads all twelve entries
UNTICKED (its rule 5) — the owner ticks them only after sealing the
route.
...
## Route A — the six offices in session (PENDING OWNER, DO NOT GENERATE)

**DO NOT GENERATE — PENDING OWNER; the route is not sealed.** (A
standalone skip marker: PromptPainter's rule 5 advises every entry below
it until the next heading, so all twelve load UNTICKED and the owner
ticks them only once Route A is chosen.)

Each window shows its office AT THE COUNCIL TABLE: a ring of six carved
thrones is visible behind and to the sides, five of them occupied by dim
unreadable silhouettes in the other five arm colours, and the window's own
office lit and acting toward the shared centre. That ring is the visual
signature that separates a Council window from its Court or Genesis
sibling, and every prompt below states it.

**PENDING OWNER — God the Judge, in session (yellow arm, 12h)** →
...
**PENDING OWNER — God the Judge, circle** →
...
**PENDING OWNER — the Devil the Destroyer, in session (orange arm, 16h)** →
...
**PENDING OWNER — the Devil the Destroyer, circle** →
...
**PENDING OWNER — the Devil the Prosecutor, in session (red arm, 20h)** →
...
**PENDING OWNER — the Devil the Prosecutor, circle** →
...
**PENDING OWNER — God the Creator, in session (moon-gray purple arm, 24h)** →
...
**PENDING OWNER — God the Creator, circle** →
...
**PENDING OWNER — Jesus the Advocate, in session (blue arm, 04h)** →
...
**PENDING OWNER — Jesus the Advocate, circle** →
...
**PENDING OWNER — Jesus the Preserver, in session (green arm, 08h)** →
...
**PENDING OWNER — Jesus the Preserver, circle** →
...
- New sheet (WORKPLAN Session 19, the Cube prompt-sheet wave, CUBE.md
  §The Double Trinity — wheel name **Council** SEALED 2026-07-26).
  **Art: 0/13** on Route A, **0/1** on Route B; nothing generated on
  either source.
- **PENDING OWNER — the ROUTE.** A = six new "in session" lancets + six
  circles (written in full above, under a standalone DO NOT GENERATE
  marker so the tool loads them unticked); B = reuse the Court's and
  Genesis's six files and generate only the Sabbath. The centre is
  generated either way.
- **PENDING OWNER — the centre's name.** *The Sabbath* is taken straight
  from CUBE.md's own reserved article layer (six offices as the six days,
  the centre as the seventh); it is a reading the canon already states,
  not an invention, but the owner has never spoken the file name.
- **Naming note carried from [Genesis Prompts](genesis_prompts.md):**
  CUBE.md's table writes **God**, the Court wheel calls the same person
  **The One** (`One_Judge`). This sheet follows CUBE.md (`God_Judge`,
  `God_Creator`); under Route B the Judge seat would point at the Court's
  existing `One_Judge.png` regardless of what the display name becomes.
- Drop root `archetypes/council` is a NEW data-driven family root in
  `tests/test_prompt_paths.py`; the wheel lands in `config.archetypes`
  with WORKPLAN Session 20.
- Verify with `python main.py "research/prompts/archetype/council_prompts.md" --dry-run`
  from `Gadgets/PromptPainter/` (13 images expected; 12 of them load
  UNTICKED by the skip-marker rule until the route is sealed).
