# Cosmos & Planets Medallion Prompts (Gemini)
...
arc and the cross-theme `dual/` convention (`research/sunday_duality.md`).
...
family sharing the star-chart border. Where Cosmos's Sun plate above
tells the *narrative* (light given away), these plates are closer to
classical planetary-medal *portraits* — each body's real, photographed
likeness translated into relief — so the two Sun prompts do not
...
### Sun eclipse (dual) (`dual/sun_eclipse.png`)
...
```
Ornate circular medallion, weathered bronze sculptural relief on dark cracked stone, photorealistic render, perfectly centered, isolated on white background. Center: the Sun in total eclipse — a perfect black disc left as flat unworked stone at the exact center, rimmed by a slender bright corona breaking free in a thin unbroken ring of light, the same silhouette the Cosmos Servant plate reads as a black hole, here read instead as the Sun simply hidden, not consumed. Border: bronze ring carved as a fine engraved star-chart — thin constellation lines linking tiny six-point stars — with four small roundels bearing the Sun glyph, mirroring the Sun plate above. Palette: darkened bronze dominant, the black disc left bare stone; the corona ring is the one element meant to hue-swap WITH the metal, exactly like the Sun plate's own corona.
```
