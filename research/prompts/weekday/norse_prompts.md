# Norse Gods — the Aesir Weekday Theme (bronze plates)

The Aesir set — Sól, Máni, Tyr, Odin, Thor, Freya, Loki, plus the
Sunday dual Skoll — is the North's mirror of the Greek theme and
predates the prompt-sheet convention entirely; this sheet regenerates
all eight plates from the canon in `Database/symbolism.json` so the
owner can drop fresh renders straight over the existing files.
...
theme's border (no meander, no hieroglyphs, no paw-prints).
