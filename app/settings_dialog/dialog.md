...
inheritance, and owns the dialog's own cross-section concerns —
...
method — the shell itself defines none of the group builders.
...
- [Location Section](location_section.md), [Display Section]
  ...
  (language_system_section.md) — the six mixins composed onto this
  shell's class statement
- [Locations](../../data/locations.md) — `LocationRepository`,
  constructed in `__init__`, released in `done`
- [Settings Store](../settings_store.md) — `Settings`/`replace`
- [Theme](../theme.md) — `apply_theme`, `size_to_screen`,
  `style_dialog_buttons`
- [Config (folder)](../../config/___config.md) — `constants`,
  `defaults`
- [UI Text Catalog](../../config/ui_text.md) — `ui()`, wrapped by `_tr`
...
- `_tr(text) -> str`: the active language's form of a chrome string
  (Phase 2) — inherited by every mixin, none of which re-declare it
...
- `result_settings() -> Settings`: the edited values as a new frozen
  Settings (valid only after Accepted) — reads instance state set by
