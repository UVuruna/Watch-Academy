...
class _DisplaySectionMixin:
    ...
    def _build_archetype_group(self) -> QGroupBox:
        ...
        # THE DAYLIGHT SWITCH (owner 2026-07-27, CUBE.md §The Rose; named
        # "Daylight - Night" by the owner 2026-07-29): the Calendar and
        # the Rose may stand in flat full color instead of the day/night
        # law — their wheels are read as a wheel first and a clock
        # second. CHECKED = the day/night cycle runs (the stored key and
        # its meaning are unchanged; only the label is the owner's new
        # wording). Inert on the other five (which always run day/night),
        # so like the two above the checkbox stays enabled and the stored
        # choice survives a pointer switch.
        self._daylight_check = QCheckBox(tr("Daylight - Night"))
