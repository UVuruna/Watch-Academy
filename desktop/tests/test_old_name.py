...
# THE THREE-FOLDER MIGRATION (2026-08-12): this guard must still cover
# EVERY shipped text file, which now spans desktop/, shared/ and the
# root docs — so PROJECT is the TRUE repo root, one level above the
# desktop Python root this file lives under (not merely desktop/).
...
ALLOWED: dict[str, tuple[str, ...]] = {
    ...
    "desktop/config/paths.py": ("a DOMY watch and a LOOP watch",),
    "desktop/config/__about/paths.md": ("DOMY watch and a LOOP watch",),
    # Not the project name: the real tree's UNTRACKED, gitignored
