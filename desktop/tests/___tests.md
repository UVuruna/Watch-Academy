...
| `test_marker_pointer.py` | THE POSITION POINTER (owner feature 2026-08-09): off by default, the setting flips `skin.year_marker.pointer_enabled`, the default/silver colors match `thumbs.shade_hue`'s own ring-finish lookup, the palette-law fallback, a real render diff near the markers, the tip never re-opens THE CLEAR ORBIT LANE's own clearance. |
| `test_encyclopedia_tree.py` | THE ENCYCLOPEDIA TREE: nine wholes seat every theme, the REACHABILITY LAW (every dial theme resolves to a seated topic bar documented look-only exceptions), no-horizontal-scroll, home 3×3 grid, variant-switcher offset, coverage law — plus THE THEME COMPLETION LAW applied to the Instrument hall: every page in the roster with a non-empty title, a non-empty article and a figure that really puts ink on the plate. |
...
| `test_moon_band.py` | THE MOON HORIZON BAND (owner verdict 2026-08-09): `core.moon.moon_horizon_arcs` geometry (both-None full circle, rise-only/set-only midnight anchoring, ordinary single arc, up-across-midnight two-arc split, golden Belgrade 2025-06-20 mockup day against `time_to_dial_angle`), settings defaults (Mode A + silver thread)/round-trip/unknown-value fallback, all 4 styles paint without error, and `build_skin` overlays `moon_band_mode`/`_style` onto `skin.year_marker`. |
...
| `test_world_mode.py` | THE TWO WORLD-MODES: Belgrade's DST star pair (−4.17°/+10.76°) recast as hand-computed BAND offsets, the equinox 180 with the solar part off, the phase flipping at Belgrade's real sunrise/sunset, both Tromsø polar regimes standing with ZERO transitions across a five-day hourly scan, Earth/Moon taking the top at night (the 2026-07-07 0.7400 moon recast), the Omega letter's hit zone following its rotated seat, the Geocentric no-op, and the flip (organic eases, a clock jump snaps, the composite never re-keys mid-move). |
