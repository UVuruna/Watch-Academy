# Sins Prompts — the Twelve Sins and the Frozen Heart (System B)

The fifth of the Dozens, and the deliberate counterpart of the Virtue
Wheel's PAINT register. **System B — the month-aligned wheel** (crown,
root, six opposition axes; see [The DOMY Canon](../../../../CANON.md),
§The Two Dozen Systems), carrying the Christian tradition's catalogue
of **SIN** — Gregory the Great, Evagrius, Aquinas, Dante. The
distinction is the whole reason both wheels exist: a **vice** is
Aristotle's measure missed, too much or too little of a good thing; a
**sin** is a bond broken. The two traditions ride the same instrument
and are never merged.

**The twelve seats — SEALED by the owner 2026-07-29** (CANON §The Sins
Dozen; nothing here waits on a later verdict):

| Hour | Sin | Argument |
|---|---|---|
| **12h — crown** | **Pride** | Gregory's root RETURNS to the rim. He lifted pride off the seven and set it over them — *"radix quippe cuncti mali superbia est"* (*Moralia* XXXI) — and Vainglory existed on his list ONLY as the seat pride had vacated; the seal folds Vainglory back INTO Pride, one seat, the stronger word. *"Pride goeth before destruction"* (Proverbs 16:18) |
...
| 16h | Violence | fortitude's ATTACK-act (the sealed 16↔04 *aggredi*) turned rabid — Dante's seventh circle, violence against neighbour, self and God, on Courage's own orange arm |
...
| 20h | Lust | the red arm's appetite in the passion hour of the early night: hunger for the body as pride is hunger for the self (Matthew 5:28) |
| 22h | Envy | the crimson wound-hour the Emotions Dozen already gives Envy, one step before the root: *"through envy of the devil came death into the world"* (Wisdom 2:24) |
| **24h — root** | **Treachery** | the root law — *the bottom of every wheel is where love breaks* — and treachery is the only sin that REQUIRES love first. Dante's ninth and deepest circle is the traitors' ice, Judas in Satan's own mouth (*Inferno* XXXIV), and the Apostles Dozen already seats Judas at 22–24h: the man and his sin end at the same midnight (John 13:18) |
| 02h | Despair | the indigo hour the Emotions give Despair — in the tradition a SIN, the sin against Hope (Aquinas, ST II-II q.20), the refusal of a mercy that was offered: Judas's second sin (Matthew 27:5) |
| 04h | Wrath | the darkest hour, where the Virtue Wheel already seats Wrath as Patience's failure — endurance snapped: *"let not the sun go down upon your wrath"* (Ephesians 4:26) |
| 06h | Idolatry | Doubt's teal hour, the divided step: *"how long halt ye between two opinions?"* (1 Kings 18:21), Elijah's challenge as the exact image the Emotions' Doubt window already draws |
| 08h | Gluttony | the green arm of Humility and renunciation, the hour of the DAILY gift: the daily bread devoured instead of shared — *"whose God is their belly"* (Philippians 3:19) |
| 10h | Acedia | Gratitude's chartreuse hour and its exact refusal: *tristitia de bono spirituali*, sorrow at one's OWN good (Aquinas, ST II-II q.35). Its famous name, the *noonday demon* (Psalm 91:6), is the one thing this seating cannot honour — noon is the crown |

**The six axes:** 12↔24 **Pride ↔ Treachery**, the crown of SELF facing
the breaking of FAITH — love of self carried to its end IS betrayal of
the other; 14↔02 the lie told to OTHERS ↔ the lie believed about
ONESELF; 16↔04 fortitude's two acts corrupted, the ATTACK turned rabid
↔ the ENDURANCE snapped; 18↔06 the two false gods, the thing HOARDED ↔
the thing WORSHIPPED (*"covetousness, which is idolatry"*, Colossians
3:5 — Paul names this axis himself); 20↔08 the two appetites of the
flesh; 22↔10 Aquinas' two sorrows, sorrow at ANOTHER's good ↔ sorrow at
ONE'S OWN. The briefs below are unchanged by the seating — each figure
is argued by its own nature, hue following that nature.

**The 12h axis reads across THREE wheels** — Magnanimity (the virtue),
Vanity (its failing), Pride (the sin): one axis of self-regard, three
readings. Note the canon's own guard: the Emotions Dozen's **PONOS at
20h** is a POLE, not a vice — *the sin is only the tip past the clear
heart* — and Pride-the-sin is that tip, a different thing from the
healthy pole.
...
[Calendar Prompts](../archetype/calendar_prompts.md)). These thirteen
are cut in the PAINT register the Virtue Wheel's vices already use:
heavy, opaque, subtractive pigment that BLOCKS the light rather than
passing it. One unifying device across all thirteen: **the FROST
RING** — a rim of creeping ice-crystal frostwork in cold pale-blue and
grey-white leadwork, thickest at the rim and reaching inward with
needle-fine crystals. **Why frost:** this wheel does not end in fire,
it ends in ICE — Dante's bottom is Cocytus, the frozen lake, and this
Dozen's own axle is the heart that FROZE, so every window on the rim
carries the cold that finishes at its center; the frost tells a sin
...
`Treachery.png`, `Despair.png`, `Wrath.png`, `Idolatry.png`,
`Gluttony.png`, `Acedia.png`, `Hardness_of_Heart.png`. Stems are the
English sin names, underscored where the name carries a space (the same
no-space rule `CalendarMount.art_stems` applies to `Just_Indignation`
on the Virtue Wheel). Figure-first: the seat a figure holds on the
wheel lives in the roster and this sheet's prose, never in the
filename.

**NOT the same set as the eight weekday `sins` emblems** (those live
under `weeks/inner_wheel/`) — this is the Dozen, a Calendar roster of
twelve plus its axle.
...
## The axle — the center that does not turn

**HARDNESS OF HEART** (*sklerokardia*) is the Sins Dozen's axle: always
present, never a rim seat (CANON §The Axle, §The Sins Dozen). It is the
**ANTI-PEACE** — the Emotions Dozen's center read the other way, and the
second of the two pairs the CENTER-PAIR LAW names: **Peace, the heart
still because it RESTS; Hardness of Heart, the heart still because it
FROZE.** Its anchors are one single scene: Jesus looks around *"being
grieved for the hardness of their hearts"* (Mark 3:5) and to those same
hardened hearts speaks the warning about the sin that *"hath never
forgiveness"* (Mark 3:29) — hardness is the MECHANISM of the
unforgivable, which is why **Blasphemy against the Spirit is ABSORBED
here rather than seated**. Pharaoh's hardened heart runs the same law
through Exodus. And the bottom of Dante's hell is ICE, not fire —
...
- New sheet, Dozen #5 (CANON §The Two Dozen Systems). **System B —
  month-aligned geometry** (wedges centered on the cardinals → one
  crown, one root, six opposition axes); the twelve sins plus the axle.
  **Art: 0/13**, none generated on either source.
- **All twelve seats and the center are SEALED (owner 2026-07-29)** —
  see the table above and CANON §The Sins Dozen. Pride crowns at 12h
  with **Vainglory FOLDED INTO IT** (one seat, the stronger word);
  Treachery roots at 24h under the root law; the 16h call, delegated to
  the orchestrator, ruled **Violence** — **Cruelty was weighed and set
  aside**, treachery containing it and carrying the deeper anchor.
- **The center is SEALED: Hardness of Heart, always present** — the
  ANTI-PEACE, with **Blasphemy against the Spirit ABSORBED** rather
  than rejected (Mark 3:5 → 3:29 are one scene). The CENTER-PAIR LAW
  (CANON §The Axle) pairs it with Peace as the two STILLNESSES, beside
  Prudence·Cunning as the two CHARIOTEERS.
- Register is the calendar-mount rondel family in the PAINT register;
  the shared device is **the FROST RING** — the wheel ends in ice, not
  fire, so the cold that finishes at its axle starts at every rim.
- Mounted this round: `sins` rides the Calendar pointer through
  `config.defaults.CALENDAR_MOUNTS`, centre `hardness_of_heart`, art
  graceful-absent until these thirteen land. Drop root `calendars/sins`
  is a NEW data-driven family root in `tests/test_prompt_paths.py`.
- Articles are a future round (the center carries no Encyclopedia entry
  yet — `constants.THIRTEENTHS` holds `None`/`None`, the documented
  graceful-absent contract).
- Verify with `python main.py "research/prompts/calendars/sins_prompts.md" --dry-run`
