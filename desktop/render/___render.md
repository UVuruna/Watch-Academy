...
| `tooltip_composer.py` | Algorithmic | THE TOOLTIP COMPOSER — the ONE DOOR: `tooltip_at` / `encyclopedia_target` / `warm_hover_articles`, the dispatch that picks the family, and the six formatting helpers they share — [about](__about/tooltip_composer.md) · [flow](__flow/tooltip_composer.md) |
| `tooltip_sky.py` | Algorithmic | every hover the SKY answers — the sun face, the moon and its lunation, the eclipses, the Earth with its season row, the twilight bands, the period window (a MIXIN on the composer, cut BY FAMILY 2026-08-19) — [about](__about/tooltip_sky.md) · [flow](__flow/tooltip_sky.md) |
| `tooltip_ring.py` | Algorithmic | every hover the RING and the ARMS answer — jewel and word legends, the live crown, the arm legend with the whole archetype/dual/thirteenth seat vocabulary — [about](__about/tooltip_ring.md) · [flow](__flow/tooltip_ring.md) |
| `tooltip_calendar.py` | Algorithmic | every hover the CALENDAR answers — the wedges and mounts, the weekday bodies, the tick readout, the zodiac/chinese/ascendant readings — [about](__about/tooltip_calendar.md) · [flow](__flow/tooltip_calendar.md) |
| `encyclopedia_targets.py` | Algorithmic | WHAT ARTICLE an element opens, never what it says — the Spacebar jump's element→(topic, index) map; works with the legend OFF — [about](__about/encyclopedia_targets.md) · [flow](__flow/encyclopedia_targets.md) |
...
| `asset_index.py` | Algorithmic | persistent stat-walk manifest — no launch reopens a file it already knows — [about](__about/asset_index.md) · [flow](__flow/asset_index.md) |
