# Cosmos Medallion Prompts (Gemini)

COSMOS is a new weekday theme (16th): each day becomes a deep-sky
object mapped to that day's own virtue/mood, the same "one theme, one
object family" pattern as wolf/bee/elephant. **Two looks, two jobs:**
the owner downloads REAL astrophotography himself for the theme's
actual art (not this document's job) — these prompts are the
**artistic medallion** register instead: aged bronze relief on dark
cracked stone, the fourth-metal-theme material wolf/greek/norse/chinese
already share so the gold/silver hue-swap keeps working unchanged,
plus a `colored/` full-color arc.

**Celestial canon (already fixed elsewhere in the project, restated
here so the two deep-sky prompt sheets agree):** the Sun is the zero
of the self-love/self-hate scale — light given away, neither exploding
nor collapsing; the **supernova** is a star destroying itself outward
(`research/bible_dark_prompts.md` names this Lucifer's death, the
self-exaltation pole); the **black hole** is a star collapsing into
itself, the light not allowed to leave (that file names it Judas's
death, sitting at the dial's bottom). COSMOS reuses both images but
maps them by day-virtue rather than by scripture-mirror: the black
hole stays on the **Sunday servant** seat (dial's bottom, same logic),
while the supernova moves to **Tuesday** for its Zeal/Wrath charge —
the two sheets are independent day-maps sharing one astrophysical
vocabulary, not the same roster twice.

## Day Mapping

| Day | Object | File stem | Virtue / Mood |
|-----|--------|-----------|----------------|
| Sun ☉ (ruler) | **Sun** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Sun.png` | the star that neither explodes nor collapses — the zero of the scale |
| Sun ☉ (servant, dual) | **Black Hole** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Black_Hole.png` | the star that fell into itself, light not allowed to leave |
| Mon ☽ | **Nebula** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Nebula.png` | Calm / Serenity — a stellar nursery |
| Tue ♂ | **Supernova** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Supernova.png` | Zeal, Courage / Wrath — the explosion |
| Wed ☿ | **Pulsar** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Pulsar.png` | the midnight mind — precise as ticking |
| Thu ♃ | **Galaxy** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Galaxy.png` | Generosity — the wheel holding billions |
| Fri ♀ | **Binary Stars** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Binary_Stars.png` | Love, the pair |
| Sat ♄ | **Comet** | `masters/weeks/celestial_bodies/cosmos/primary/bronze/Comet.png` | Patience / Renewal — the returning wanderer |
...
no paw-prints (wolf), no meander (greek), no huiwen (chinese). The
Cosmos ring is **a fine engraved star-chart — thin constellation
lines linking tiny six-point stars** — with four small roundels
bearing the day's planetary glyph, same roundel convention every
other bronze theme uses.

**Metal-swap note (technical), same rule as wolf:** gold/silver
styles hue-swap the WARM BRONZE pixels (hue window ~10–60°); every
non-metal accent in the prompts below is kept COOL or RED, below that
window. The deliberate exception is the bright ring/beam that carries
each plate's "light" reading — the Sun's streaming rays, the black
hole's accretion ring, the eclipse corona — each called out below to
hue-swap WITH the metal, exactly like the wolf Alpha/Omega sun-and-
corona pair.

**File-stem rule (deviates from wolf on purpose):** wolf/greek/norse
capitalize stems because those themes name proper nouns (gods,
named wolves). Cosmos objects are common nouns, so stems stay
lowercase snake_case — the same convention `bible_theme_prompts.md`
and `bible_dark_prompts.md` already use for their casts.

**Drop (wiring follows once the art lands):**
- `assets/weeks/celestial_bodies/cosmos/primary/` — `masters/weeks/celestial_bodies/cosmos/primary/bronze/Sun.png`, `masters/weeks/celestial_bodies/cosmos/primary/bronze/Nebula.png`, `masters/weeks/celestial_bodies/cosmos/primary/bronze/Supernova.png`,
  `masters/weeks/celestial_bodies/cosmos/primary/bronze/Pulsar.png`, `masters/weeks/celestial_bodies/cosmos/primary/bronze/Galaxy.png`, `masters/weeks/celestial_bodies/cosmos/primary/bronze/Binary_Stars.png`, `masters/weeks/celestial_bodies/cosmos/primary/bronze/Comet.png`
- `masters/weeks/celestial_bodies/cosmos/primary/bronze/Black_Hole.png`
- `assets/weeks/celestial_bodies/cosmos/primary/colored/` — same seven stems
- `masters/weeks/celestial_bodies/cosmos/primary/colored/Black_Hole.png`

---

## Bronze plates

### Sunday, ruler face — the Sun (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Sun.png`)
...
### Sunday, servant face (dual) — Black Hole (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Black_Hole.png`)
...
### Monday — Nebula (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Nebula.png`)
...
### Tuesday — Supernova (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Supernova.png`)
...
### Wednesday — Pulsar (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Pulsar.png`)
...
### Thursday — Galaxy (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Galaxy.png`)
...
### Friday — Binary Stars (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Binary_Stars.png`)
...
### Saturday — Comet (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Comet.png`)

Patience/Renewal: the returning wanderer, decades between one sight
of its sun and the next.

```
Ornate circular medallion, weathered bronze sculptural relief on dark cracked stone, photorealistic render, perfectly centered, isolated on white background. Center: a small icy wanderer arcing across the plate, a long low-relief tail streaming behind it, and at the far edge of the disc a tiny distant sun it is only now returning to after decades away in the dark — the whole composition reading as one long patient arc. Border: bronze ring carved as a fine engraved star-chart — thin constellation lines linking tiny six-point stars — with four small roundels bearing the Saturn glyph. Palette: darkened aged bronze dominant; the only bright accent the pale glint of the distant sun and the comet's frost-lit tail.
```

---

## Colored plates — painted cosmos, one chat session for consistency

Not a recolor: each plate is a fresh full-color painting of the same
composition, same convention as wolf's colored arc. Generate the
whole set together so the series stays visually consistent.

### Sunday, ruler face — the Sun (`masters/weeks/celestial_bodies/cosmos/primary/colored/Sun.png`)
...
### Sunday, servant face (dual) — Black Hole (`masters/weeks/celestial_bodies/cosmos/primary/colored/Black_Hole.png`)
...
### Monday — Nebula (`masters/weeks/celestial_bodies/cosmos/primary/colored/Nebula.png`)
...
### Tuesday — Supernova (`masters/weeks/celestial_bodies/cosmos/primary/colored/Supernova.png`)
...
### Wednesday — Pulsar (`masters/weeks/celestial_bodies/cosmos/primary/colored/Pulsar.png`)
...
### Thursday — Galaxy (`masters/weeks/celestial_bodies/cosmos/primary/colored/Galaxy.png`)
...
### Friday — Binary Stars (`masters/weeks/celestial_bodies/cosmos/primary/colored/Binary_Stars.png`)
...
### Saturday — Comet (`masters/weeks/celestial_bodies/cosmos/primary/colored/Comet.png`)
...
The Planets ART medallion look lives in its own sheet:
[Planets ART Medallion Prompts](planet_art_prompts.md).

---

## Delivery note

Same pipeline as the rest of the project's generated art: Gemini's
PNG output drops to the project root, then goes through
white-background removal + circle crop + resize to 800×800 RGBA,
same as every other weekday/badge asset delivered so far
(`research/bible_dark_prompts.md`, `research/instrument_prompts.md`).

---

## The Ninth — the Big Bang (`masters/weeks/celestial_bodies/cosmos/primary/bronze/Big_Bang.png`)
...
**The Big Bang (colored)** → `masters/weeks/celestial_bodies/cosmos/primary/colored/Big_Bang.png`
```
