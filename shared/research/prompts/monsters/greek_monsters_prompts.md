# Greek Monsters — the Bestiary Weekday Theme (Gemini/ChatGPT)

Owner-approved 2026-07-21 night: a NEW weekday theme, single roster (no
Planetary/Pantheon split — the six weekday seats, a Sunday dual, a
Ninth and a title plate, the same "closed set" shape wolf/bee/elephant/
cosmos already ship). The cast sits by NATURE against the seat's own
color/virtue/vice bundle (CANON.md "The Web of Correspondences"), never
by a literal planetary match — Medusa is not "the moon," she is
Monday's FEAR made flesh.

**Register — a darker sibling of the Olympian family:** the god
medallions (`weekday/greek_prompts.md`) wear aged-bronze relief and
Mediterranean enamel; this bestiary wears the house **night-window**
stained-glass register instead (the same family `era_prompts.md` and
`months_prompts.md` use) — dark jewel-toned glass, the circular window
shape IS the frame — while KEEPING the Olympian family's own **Greek
key (meander) border**, recut as dark leadwork instead of bronze relief
or polished gold. One glance says "Greek"; the dark glass says
"monster," not "god." Every plate carries a `primary/` night-window
version and a `colored/` full-vivid-enamel version, same two-register
convention every other weekday theme ships (owner: "night-window +
colored variants per the theme-family conventions").

**Derivation check (Rule #19):** every plate below is an irreducible
scene — a different figure, a different myth-beat, no two plates share
a composition a formula could derive one from the other. The
`primary`/`colored` split is NOT a tint pass (unlike gold/silver
metal-swap themes): colored is a fresh full-color repaint of the same
scene, the same non-derivable convention every sibling sheet already
uses (`greek_prompts.md`, `cosmos_prompts.md`). Nothing here is a
...
one documented exception in this whole project — see
[Theme Title Prompts](../titles/theme_title_prompts.md) — and even it
lives in a SEPARATE file/root, not here).

**Drop paths:** `masters/weeks/myth/age_of_heroes/primary/bronze/<Stem>.png` (night-
window) and `masters/weeks/myth/age_of_heroes/primary/colored/<Stem>.png` (full enamel)
...
|---|---|---|
| Monday | blue · Fear/Serenity | **Medusa** |
| Tuesday | orange · Wrath/Courage | **Minotaur** |
| Wednesday | purple · Greed/Wisdom | **Sphinx** |
| Thursday | yellow · Excess/Generosity | **Erymanthian Boar** |
| Friday | red · Jealousy/Love | **Sirens** |
| Saturday | green · Envy/Patience | **Hydra** |
| Sunday (Ruler) | white-gold · Pride/Justice | **Nemean Lion** |
| Sunday (Servant, dual) | black-silver · Servility/Humility | **Cerberus** |
| Ninth (Excluded) | — | **Pegasus** |
| Title plate | — | **Typhon & Echidna** |

The dual is a blood pair, not an allegory: Nemean Lion and Cerberus are
literally BROTHERS, both children of Typhon and Echidna — one guards
the sun's own labor (Heracles' first), the other guards the midnight
door. The Ninth, Pegasus, is the **Excluded** kind (CANON.md "The
Ninth"): born from Medusa's death, he belongs to the myth-web but sits
in neither the monster line nor the god line — "neither monster nor
god, belonging to no world."

---

## Monday — Medusa (`masters/weeks/myth/age_of_heroes/primary/bronze/Medusa.png`)
...
plate's own center, exactly the myth's own warning.*
...
**Medusa (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Medusa.png`
```
...
## Tuesday — Minotaur (`masters/weeks/myth/age_of_heroes/primary/bronze/Minotaur.png`)
...
**Minotaur (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Minotaur.png`
```
...
## Wednesday — Sphinx (`masters/weeks/myth/age_of_heroes/primary/bronze/Sphinx.png`)
...
**Sphinx (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Sphinx.png`
```
...
## Thursday — Erymanthian Boar (`masters/weeks/myth/age_of_heroes/primary/bronze/Erymanthian_Boar.png`)
...
**Erymanthian Boar (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Erymanthian_Boar.png`
```
...
## Friday — Sirens (`masters/weeks/myth/age_of_heroes/primary/bronze/Sirens.png`)
...
**Sirens (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Sirens.png`
```
...
## Saturday — Hydra (`masters/weeks/myth/age_of_heroes/primary/bronze/Hydra.png`)
...
**Hydra (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Hydra.png`
```
...
## Sunday — Nemean Lion (Ruler face, `masters/weeks/myth/age_of_heroes/primary/bronze/Nemean_Lion.png`)
...
**Nemean Lion (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Nemean_Lion.png`
```
...
## Sunday — Cerberus (Servant face, dual, `masters/weeks/myth/age_of_heroes/primary/bronze/Cerberus.png`)
...
**Cerberus (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Cerberus.png`
```
...
## The Ninth (Excluded) — Pegasus (`masters/weeks/myth/age_of_heroes/primary/bronze/Pegasus.png`)
...
than the seated eight (CANON.md).*
...
**Pegasus (colored)** → `masters/weeks/myth/age_of_heroes/primary/colored/Pegasus.png`
```
...
**Typhon & Echidna** (the parents of the roster, father of monsters
whose children now hold the week) is this theme's TITLE PLATE — the
whole-theme summary image every weekday theme carries a slot for
(`app.encyclopedia._weekday_topic`'s `title_entry`, currently
graceful-absent for every theme). Per the canonical title-plate root
...
(`masters/weeks/societies/wolf/primary/colored/<theme>.png`), separate from the weekday cast root,
...
- New sheet (R8c PROMPT SHEETS round, owner-approved 2026-07-21 night).
  Theme not yet registered in `config/defaults.py`
  ...
  (per this round's own brief: "Sheets ONLY... NO app code").
  ...
  entry needed, exactly like every other weekday theme's sheet.
- **Art: 0/18** (9 figures × 2 registers; the title plate is tracked
  separately, see above).
- Verify with `python main.py "research/prompts/monsters/greek_monsters_prompts.md" --dry-run`
