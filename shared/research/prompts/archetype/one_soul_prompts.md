# Prism Archetype Prompts (Gemini) — One Soul, the Bond, the six pillars of a home

The PRISM LIGHT pointer's archetype set (owner APPROVED 2026-07-16,
canon in [The DOMY Canon](../../../../CANON.md), theme definition in
[The Bond](../../bond_theme.md)): where the Prism PAINT hexagram
carries heaven's six persons, the LIGHT hexagram carries the six
pillars of a home — the same wheel-web spoken in the dual. The
theme's engine is the owner's own Conjugation Law: *"honesty is a
trait of the individual; trust is its counterpart in the
relationship"* — every pillar is a seat's VIRTUE conjugated between
two persons, and every pillar's SHADOW is that seat's VICE conjugated
the same way. So each window carries the TWO-ROW canon VISUALLY: the
...
magenta** (24h — purple's light neighbour, the midnight-talks seat),
**Trust blue** (04h — sleeping soundly beside someone), **Respect
cyan** (08h — the cool clear air you leave around what you respect).

The cross-cures conjugate on the diagonals, as always: **Trust
disarms the Fight and Support cures Suspicion** (04h↔16h);
**Gratitude cures Score-keeping and Tolerance cures Taking-for-
granted** (12h↔24h); **Passion burns away Contempt and Respect calms
Jealousy** (20h↔08h). The center is the UNION — Zajedništvo, two made
one flesh (Genesis 2:24) — and the Ninth is the Child, the threefold
cord (Ecclesiastes 4:12). Both scriptures live in the ARTICLES only;
the house rule stands — images never carry lettering.

**Color fidelity** (canon, owner 2026-07-16): each re-hue is argued,
never at random — Gratitude takes green as GROWTH (the highest love
made visible as a living thing), Support takes yellow as the working
DAYLIGHT one becomes for the other, Respect takes cyan as the clean
COOL AIR it keeps around what it honours, Tolerance takes magenta as
purple's light neighbour (Wisdom's hue softened for the home). Trust
keeps blue and Passion keeps red — their planet colors unchanged.

**Register:** the stained-glass night-window family — the same
register as the Scale glass and the Trinity/Persons lancets (see
[Scale Prompts](../badge/scale_badge_prompts.md),
[Persons Prompts](persons_prompts.md)); each pillar is a LANCET
...
Taking-for-granted / Podrazumevanje) → `masters/archetypes/one_soul/primary/colored/Gratitude.png`
...
`masters/archetypes/one_soul/primary/colored/Support.png`
...
Jealousy) → `masters/archetypes/one_soul/primary/colored/Passion.png`
...
Sebičnost) → `masters/archetypes/one_soul/primary/colored/Tolerance.png`
...
→ `masters/archetypes/one_soul/primary/colored/Trust.png`
...
`masters/archetypes/one_soul/primary/colored/Respect.png`
...
**The Union (Zajedništvo)** → `masters/archetypes/one_soul/primary/colored/Union.png`
...
other, the throne's dual nature conjugated between two into one flesh,
Genesis 2:24. The center's clear-glass heart echoes the Eye and the
Seal — the union does not color the light.)*

```
ROUND rose window — a circular stained-glass rosette, night-window register, photorealistic render, isolated background. Center: TWO figures facing each other, each at once a servant AND a king — each wears a crown yet KNEELS slightly to the other, and each reaches out to set a crown upon the other's head, their four hands forming a woven ring at the middle of the window; where their hands cross, the glass turns PERFECTLY CLEAR and uncolored — plain white daylight passes through the exact center, the brightest point of the window, the one flesh they become. One figure is glazed in warm rose-gold, the other in cool blue-silver, meeting seamlessly at the center seam. Straight rays of soft light burst from the clasped hands through rings of gentle cloudglass to the rim. Rim: gothic stone tracery; four rim roundels evenly placed — two joined rings at the apex, a crown offered outward at each side, a single hearth-flame at the bottom. Palette: warm rose-gold and cool blue-silver in equal measure, black-iron lead, the clasped-hands center the only uncolored glass. NO lettering anywhere.
```
...
**The Child** → `masters/archetypes/one_soul/primary/colored/Child.png`
...
```
SMALL round stained-glass rondel, night-window register, photorealistic render, isolated background. Center: a small child held safe between two larger figures who lean in from either side to form a sheltering arch over it — the three heads together making a close triangle, warm light gathered at the child at the middle. Woven through and around the three, a single CORD of three strands braided into one, running the full ring of the rondel unbroken. The child glazed in bright new spring-green, the two elders in warm rose-gold and cool blue-silver, so the three colours of the family meet at the center. Border: living-vine leadwork; three tiny rim roundels — a braided three-strand cord at the apex, a sheltering pair of hands at one side, a small green shoot at the other. Palette: spring green, rose-gold, blue-silver, soft white light, green-black lead. NO lettering anywhere.
```
...
- Owner canon: One Soul, the Bond seated on the Prism LIGHT pointer
  APPROVED 2026-07-16; the light hues and the shadow duality confirmed
  the same day (canon, [The DOMY Canon](../../../../CANON.md); theme,
  [The Bond](../../bond_theme.md)).
- The theme NAME is picked along the way from the ★-marked list — the
  agent's pick is **One Soul (Jedna duša)** (Aristotle, "one soul
  dwelling in two bodies"); the file uses it. If the owner settles on
  The Vow or The Bond, only the sheet title changes, not the plates.
- The Prism PAINT set — The Persons — has its own sheet:
  [Persons Prompts](persons_prompts.md).
- Open with the owner: whether the Union's two figures should be
  shown faceless (as the Trinity Judge is veiled) or plainly human —
  DO NOT decide unprompted.
