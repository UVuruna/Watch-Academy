# Banknote Offices Prompts — the Fivefold Code of the Dollar Ring

The DOLLAR/EYE + CROSS-WORDS rounds (owner decrees 2026-07-27,
CANON.md §The Banknote — The Fivefold Offices): the Dollar ring's
five letters read as the Double Trinity's OFFICES, and the hover
legend names them — **M** Malignant Accuser, **A** Anointed Aegis,
**S** Satanic Scourge, **Ω** Omnific Originator, **N** Nazarene
Advocate. The sixth seat, the Eye of Providence (God as Judge),
already has its art — the four `Eye[_shine]_gem/_gpt` masters in the
ring letter library — and gets NO entry here (one image, one place).

The compressed formula the images serve: *the Accuser condemns, the
Aegis protects, the Scourge destroys, the Originator creates, the
Advocate defends — and the Judge watches.*

| Office | Letter | Seat | Person | Function |
|---|---|---|---|---|
| **Malignant Accuser** | M | 20h red | the Devil | Prosecutor — indicts the soul (Revelation 12:10) |
| **Anointed Aegis** | A | 08h green | Jesus | Guardian — the shield that takes the blow (Hebrews 1:14) |
| **Satanic Scourge** | S | 16h orange | the Devil | Destroyer — wastes what it cannot own |
| **Omnific Originator** | Ω | 24h purple-gray | God | Creator — gives existence to existence (Revelation 1:8) |
| **Nazarene Advocate** | N | 04h blue | Jesus | Advocate — answers the charge (1 John 2:1) |

**Register & look — the BANKNOTE ENGRAVING family (new root):**
these are the Dollar ring's own plates, so they wear the note's own
craft — **engraved intaglio banknote style**, fine line hatching and
guilloche ornament, dollar-green ink on aged cream paper, NOT the
stained-glass night-window family. Family root
`assets/archetypes/banknote/`, register **primary**, look **colored**
(the tree law, `assets/___assets.md`). Every image is **1:1**. Sheets
state canonical SOURCE-LESS paths; PromptPainter appends the
`_gem`/`_gpt` suffix.

**Drop path:** `assets/archetypes/banknote/primary/colored/`. Stems,
PascalCase with the word separator (the 0.14.485 rule):
`Malignant_Accuser`, `Anointed_Aegis`, `Satanic_Scourge`,
`Omnific_Originator`, `Nazarene_Advocate`.

---

## THE HOUSE LETTERING RULE — read before generating

**NO lettering inside any image** — and on THIS family the rule needs
shouting, because real banknote engravings are covered in text:
no denominations, no crown texts, no ANNUIT COEPTIS, no MASON letters, no
scrollwork words. The crown texts already ring the dial as the ring's own
arc text; the plates carry only figure and ornament.

---

## Rule #19 derivation check (written, root `CLAUDE.md` Rule #19)

**IRREDUCIBLE — generated (5 files):** five office personifications in
a style (banknote intaglio) that exists nowhere in the project. Not
derivable from any master: the Double Trinity's Court/Genesis/Council
wheels tell the SAME persons in the stained-glass register with their
own sealed "in session" tellings — a banknote engraving of an office
is a different composition in a different craft, and the
one-image-one-place law forbids sharing files with those wheels
anyway.

**COMPUTED — never generated:** the dollar-green (and any other)
tint — `dollar_green` is a RAMP in `recolor/presets/metals.json` and
the Thematic finish recolors live; metals, opacity, scaling —
transform math. The Eye of Providence — ALREADY GENERATED (the ring
letter library's four Eye masters); adding a fifth eye here would be
exactly the duplication Rule #19 forbids.

**REUSE — none.** Checked: Court (`One_Judge`…), Genesis
(`God_Creator`…) and Council sheets carry these persons' OFFICES in
glass; no file is shared by design (see above).
...
- New sheet (CROSS-WORDS round, owner UV inbox 2026-07-27 — the
  Dollar legend's five offices). **Art: 0/5**; nothing generated on
  either source.
- **NEW family root** `archetypes/banknote` — registered as a
  data-driven root in `tests/test_prompt_paths.py` (wiring lands with
...
- Verify with `python main.py "research/prompts/archetype/banknote_offices_prompts.md" --dry-run`
  from `Gadgets/PromptPainter/` (5 images expected).
