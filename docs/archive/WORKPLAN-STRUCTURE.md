...
session repeats**; [Refactor God-Files](../../../../rules/briefs/REFACTOR-GODFILES.md);
...
| `config/dial.py` | "Window", "Procedural FALLBACK geometry", "Moon/Earth rim transit", "Ring faces" (incl. the ring-tint swatch-size leftovers — the tint HUES are palette's), "Hand sizing", + `OMEGA_HIT_RADIUS_FRACTION` (the named exception: it is dial HIT geometry computed from `RING_JEWEL_ART_SCALE`, so it moves here, out of the Session 27 section) | dial geometry and window sizing |
